...
let esDone = false;
const client = new elasticsearch_1.Client(config.elasticsearch);
const mustNot = [
...
function addURL(item) {
    ...
    file.dest = DEST_FOLDER + utils_1.pathFixer(file.url.replace(/^https?:\/\//, ''));
    utils_1.mkdirpFor(file.dest);
    fs_1.stat(file.dest, (err, stat) => {
...
function downloadDone(file, success, fileDeleted = false) {
    if (success === RES_SKIP) {
        skippedCount++;
    }
    else if (success) {
...
function downloadNext() {
    ...
    inProgress++;
    const out = fs_1.createWriteStream(file.dest);
    https_1.request(file.url, { agent }, (res) => {
